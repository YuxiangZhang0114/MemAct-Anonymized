...
class AsyncvLLMServer(AsyncServerBase):
    ...
    async def generate(self, prompt_ids: list[int], sampling_params: dict[str, Any], request_id: str) -> list[int]:
        # Compute the remaining token budget for this request based on max_model_len
        budget_max_tokens = self.max_model_len - len(prompt_ids)

        # Defensive copy and unify potential user-specified length keys
        # to avoid passing duplicate max_tokens to SamplingParams
        sp = dict(sampling_params or {})

        user_max_tokens: Optional[int] = None
        if "max_tokens" in sp:
            user_max_tokens = sp.pop("max_tokens")
        # Some callers may use "max_new_tokens"; treat it as an alias if provided
        if "max_new_tokens" in sp:
            # If both are provided, prefer explicit max_tokens and drop max_new_tokens
            if user_max_tokens is None:
                user_max_tokens = sp.pop("max_new_tokens")
            else:
                sp.pop("max_new_tokens", None)

        try:
            user_max_tokens_int = int(user_max_tokens) if user_max_tokens is not None else None
        except Exception:
            user_max_tokens_int = None

        # Final max_tokens respects both model budget and user limit (if any)
        if user_max_tokens_int is not None:
            max_tokens = min(budget_max_tokens, max(0, user_max_tokens_int))
        else:
            max_tokens = budget_max_tokens

        sampling_params = SamplingParams(max_tokens=max_tokens, **sp)
