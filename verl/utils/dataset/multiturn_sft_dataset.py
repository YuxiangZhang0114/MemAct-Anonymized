...
def normalize_enable_thinking(value: Any) -> Optional[bool]:
    """
    Coerce arbitrary values loaded from parquet into a Python bool or None for enable_thinking.

    This avoids Jinja evaluating numpy arrays in `{% if enable_thinking %}`, which raises
    "The truth value of an array with more than one element is ambiguous".
    """
    # Treat explicit None/NaN as None
    try:
        import pandas as _pd  # local import to avoid global dependency at module import time
    except Exception:
        _pd = None
    ...
    if isinstance(value, np.ndarray):
        ...
        # If it's an unexpected multi-element array, default to True if any element is truthy
        return bool(np.any(value))

    # List/tuple: use first element if present, otherwise None
    if isinstance(value, (list, tuple)):
        if len(value) == 0:
            return None
        return normalize_enable_thinking(value[0])

    # Fallback: best-effort boolean cast
    try:
        return bool(value)
    except Exception:
        return None
...
class MultiTurnSFTDataset(Dataset):
    ...
    def _read_files_and_process(self):
        ...
        # Extract messages list from dataframe, ensure no numpy arrays remain
        self.messages = (
        ...
        if self.tools_key in self.dataframe.columns:
            ...
            self.tools = None
        # Extract enable_thinking list from dataframe, coerce to Python bool/None
        if self.enable_thinking_key in self.dataframe.columns:
            self.enable_thinking = (
                self.dataframe[self.enable_thinking_key]
                .apply(series_to_item)
                .apply(normalize_enable_thinking)
                .tolist()
            )
        else:
    ...
    def _process_message_tokens(
        ...
        should_mask_assistant: bool = False,
        ...
        Args:
            ...
            should_mask_assistant: Whether to mask this assistant message
        Returns:
            Tuple of (tokens, loss_mask, attention_mask)
        """
        # should_mask_assistant = False
        if start_idx > 0:
        ...
        if is_assistant:
            ...
            loss_mask = [0] * (len(generation_prompt_tokens)) + [1 if not should_mask_assistant else 0] * (
    ...
    def _validate_and_convert_tokens(
        ...
        if len(concat_tokens) != len(full_tokens_list) or not all(
            ...
            with open('token_mismatch.txt', 'a') as f:
                f.write(f"Token mismatch detected! Full tokenization length: {len(full_tokens_list)}, Concatenated tokens "
                        f"length: {len(concat_tokens)}. Using concatenated version.\n")

            #     f.write(f"full tokens text: {self.tokenizer.decode(full_tokens_list)}\n")
            #     f.write(f"concat tokens text: {self.tokenizer.decode(concat_tokens)}\n")
            #     exit()
            # logging.warning(
    ...
    def __getitem__(self, item):
        ...
        for i in range(len(messages)):
            if messages[i]["role"] == "assistant":
                if 'tool_calls' in messages[i] and messages[i]["tool_calls"] is not None:
                    # Remove None values from arguments
                    for tool_call in messages[i]["tool_calls"]:
                        tool_call["function"]["arguments"] = {k: v for k, v in tool_call["function"]["arguments"].items() if v is not None}

                        if 'memory' in tool_call["function"]["arguments"]:
                            memory = tool_call["function"]["arguments"].get("memory", "")
                            delete_ids = tool_call["function"]["arguments"].get("delete_ids", [])
                            del tool_call["function"]["arguments"]["memory"]
                            del tool_call["function"]["arguments"]["delete_ids"]
                            # Reassign to preserve order
                            tool_call["function"]["arguments"]["memory"] = memory
                            tool_call["function"]["arguments"]["delete_ids"] = delete_ids
                        
                        elif 'goal' in tool_call["function"]["arguments"]:
                            goal = tool_call["function"]["arguments"].get("goal", "")
                            del tool_call["function"]["arguments"]["goal"]
                            tool_call["function"]["arguments"]["goal"] = goal
                            
            
        is_end_of_conversation = False
        if messages[-1]["role"] == "assistant" and messages[-1]["tool_calls"] is None:
            is_end_of_conversation = True
        
        pruned_tool_messages_idxs = []
        for idx, cur_messages in enumerate(messages):
            # if cur_messages["role"] == "tool" and 'prune_context' in cur_messages["cont"]:
            #     pruned_tool_messages_idxs.append(idx)

            if cur_messages["role"] == "assistant":
                if not isinstance(cur_messages.get("tool_calls", []), list):
                    continue
                for tool_call in cur_messages.get("tool_calls", []):
                    if tool_call["function"]["name"] == "prune_context":
                        pruned_tool_messages_idxs.append(idx)
                        break
            
        
        if is_end_of_conversation:
            if len(pruned_tool_messages_idxs) > 0:
                mask_before_idx = pruned_tool_messages_idxs[-1]
            else:
                mask_before_idx = 0
        else:   
            if len(pruned_tool_messages_idxs) >1:
                mask_before_idx = pruned_tool_messages_idxs[-2]
            else:
                mask_before_idx = 0

        
        # Masking strategy update for assistant messages based on existing foundation
        ...
        while i < len(messages):
            ...
            if cur_messages["role"] == "assistant":
                ...
                tokens, loss_mask, attention_mask = self._process_message_tokens(
                    ...
                    should_mask_assistant= True if i <= mask_before_idx else False
                )
