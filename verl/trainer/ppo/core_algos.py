...
@register_adv_est(AdvantageEstimator.DCPO)  # or simply: @register_adv_est("dcpo")
def compute_dcpo_outcome_advantage(
    ...
    """
    Compute advantage for DCPO, operating only on Outcome reward
    (with only one scalar reward for each response). Deduplicate by rollout_ids
    within each group when computing group statistics.
    ...
    Args:
        ...
        index: `(np.ndarray)`
            index array for grouping
        rollout_ids: `(np.ndarray)`
            rollout_id array for rollout segment grouping
        epsilon: `(float)`
            small value to avoid division by zero
        norm_adv_by_std_in_grpo: `(bool)`
            whether to scale the GRPO advantage
    ...
    with torch.no_grad():
        bsz = scores.shape[0]
        # Deduplicate by rollout_ids within each group: only the first occurrence per rollout_id counts
        id2seen_rollouts = defaultdict(set)
        for i in range(bsz):
            grp = index[i]
            rid = rollout_ids[i]
            if rid not in id2seen_rollouts[grp]:
                id2score[grp].append(scores[i])
                id2seen_rollouts[grp].add(rid)
        for idx in id2score:
            if len(id2score[idx]) == 1:
                id2mean[idx] = torch.tensor(0.0)
                id2std[idx] = torch.tensor(1.0)
