...
def _passages2string(retrieval_result):
    ...
    for idx, doc_item in enumerate(retrieval_result):
        ...
        text = "\n".join(content.split("\n")[1:])
        format_reference += f"Doc {idx + 1} (Title: {title})\n{text}\nurl: {doc_item['document'].get('url', 'No URL')}\n\n"
    return format_reference.strip()


def generate_short_uuid(length: int = 8) -> str:
    """
    Generate a short UUID with default length of 8 characters.
    
    Args:
        length: UUID length, default is 8 characters
...
def perform_single_search_batch(
    ...
    request_uuid = generate_short_uuid()
    if error_msg:
        ...
        try:
            ...
            if raw_results:
                ...
                # final_result = "\n---\n".join(pretty_results)
                # result_text = json.dumps({"success": True, "result": final_result, "tool_call_id": request_uuid})
                ...
                for retrieval in raw_results:
                    if 'id' in retrieval:
                        del retrieval['id']
                    if 'wikipedia_id' in retrieval:
                        del retrieval['wikipedia_id']

                    # if 'wikipedia_title' in retrieval:
                    #     retrieval['title'] = retrieval.pop('wikipedia_title')
                # raw_results=json.dumps(raw_results, ensure_ascii=False)
                result_text = json.dumps({"success": True, "tool_call_id": request_uuid, "result": raw_results})
                logger.info(f"Batch search: Successful")
            else:
                result_text = json.dumps({"success": True, "tool_call_id": request_uuid, "result": "No search results found."})
                metadata["status"] = "no_results"
                metadata["total_results"] = 0
                logger.info("Batch search: No results found")
        except Exception as e:
            error_msg = f"Error processing search 12 results: {e}"
            result_text = json.dumps({"success": False, "tool_call_id": request_uuid, "result": error_msg})
            metadata["status"] = "processing_error"
            logger.error(f"Batch search: {error_msg}")
    else:
        metadata["status"] = "unknown_api_state"
        result_text = json.dumps({"success": False, "tool_call_id": request_uuid, "result": "Unknown API state (no response and no error message)."})
        logger.error("Batch search: Unknown API state.")

    # print(f"Batch search: Result text: {result_text}")
