#!/bin/bash
# DCPO (Direct Contrastive Preference Optimization) training script for 14B model
# This script trains a 14B model using DCPO algorithm on a single node with 8x H100 GPUs
# Note: Uses sequence parallel size of 2 for larger model
#
# Usage:
...
#   4. Run: bash run_dcpo_14B_single.sh
...
# Model configuration
model_size="14B"
model_path=/path/to/model  # Path to pretrained 14B model
...
# Project and logging
project_name="memagent_dcpo_single_node_14B"
logdir="/path/to/logs/${project_name}"
mkdir -p ${logdir}
logpath="${logdir}/master.log"

# Data paths
train_single=$PROJECT_DIR/DCPO/data/wiki_train_8192.parquet
val_single=$PROJECT_DIR/DCPO/data/wiki_val_512.parquet
train_multi_object=$PROJECT_DIR/DCPO/data/wiki_train_multi_object_2_3.parquet
val_multi_object=$PROJECT_DIR/DCPO/data/wiki_val_multi_object_2_3.parquet

train_files="['$train_single', '$train_multi_object']"
test_files="['$val_single','$val_multi_object']"

# Tool configuration
tool_config_path=$PROJECT_DIR/Mem/config/mem_search_tool_config_single.yaml
